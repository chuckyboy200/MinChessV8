...
public static long some(int positionFirst, int positionLast) {
    ...
    for(int positionNumber = positionFirst; positionNumber <= positionLast; positionNumber ++) {
        ...
        System.out.println(POSITION_FEN[positionNumber]);
...
public static long perftSearch(int depth, int maxDepth, int maxNum, Board board) {
    if(depth == 0) return 1;
    long positions = 0;
    board = Board.gen(board, false);
    ...
    for(int i = 0; i < board.moveSize(); i ++) {
        ...
        if(depth == maxDepth) {
            System.out.print(" " + ((++ num <= 9) ? " " : "") + num + "/" + maxNum + " " + board.moveAlgebraic(i) + ": ");
        }
