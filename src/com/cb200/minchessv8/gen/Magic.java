...
public class Magic {
	...
	/**
	 * Get all rook moves with magic bitboards
	 *
	 * @param square The rook square
	 * @param allOccupancy The bitboard for all occupied squares
	 * @return A bitboard containing all valid rook moves
	 */
	...
	/**
	 * Get all bishop moves with magic bitboards
	 *
	 * @param square The bishop square
	 * @param allOccupancy The bitboard for all occupied squares
	 * @return A bitboard containing all valid bishop moves
	 */
	...
	/**
	 * Get all queen moves with magic bitboards
	 *
	 * @param square The queen square
	 * @param allOccupancy The bitboard for all occupied squares
	 * @return A bitboard containing all valid queen moves
	 */
	...
	// generate all rook and bishop magics
	static {
		for (int square = 0; square < 64; square ++) {
			int rank = square >>> 3;
			int file = square & 7;
			ROOK_MOVEMENT[square] = (B.FILE[file] | B.RANK[rank]) & ~((file == 0 ? 0 : B.FILE[0]) | (file == 7 ? 0 : B.FILE[7]) | (rank == 0 ? 0 : B.RANK[0]) | (rank == 7 ? 0 : B.RANK[7]) | (1L << square));
			ROOK_SHIFT[square] = 64 - Long.bitCount(ROOK_MOVEMENT[square]);
			ROOK_MOVES[square] = generateRookMoves(square, ROOK_SHIFT[square]);
			BISHOP_MOVEMENT[square] = ((B.DIAG[B.DIAG_INDEX[0][square]] | B.DIAG[B.DIAG_INDEX[1][square]]) ^ (1L << square)) & ~OUTER;
		...
		for (int variationIndex = 1; variationIndex < variationCount; variationIndex++) {
			...
			for (int i = 0; i < 32 - Integer.numberOfLeadingZeros(variationIndex); i++) {
				if (((1L << i) & variationIndex) != 0) {
					occupancyVariations[variationIndex] |= Long.lowestOneBit(currentMask);
				}
